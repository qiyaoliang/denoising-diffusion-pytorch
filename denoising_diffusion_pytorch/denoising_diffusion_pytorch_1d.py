...
class Unet1D(nn.Module):
    def __init__(
        ...
        # need to modify this time dim to include the dimension of the other conditional embeddings
        time_dim = dim * 4
        ...
        self.features_mlp1 = nn.Sequential(
            sinu_pos_emb,
            nn.Linear(fourier_dim, time_dim),
            nn.GELU(),
            nn.Linear(time_dim, time_dim)
        ...
        self.features_mlp2 = nn.Sequential(
            sinu_pos_emb,
            nn.Linear(fourier_dim, time_dim),
            nn.GELU(),
            nn.Linear(time_dim, time_dim)
    ...
    def forward(self, x, time, x_self_cond=None, labels=None):
        ...
        if labels is not None:
            feature1 = self.features_mlp1(labels[0].to(device=x.device))
            feature2 = self.features_mlp2(labels[1].to(device=x.device))
            t = torch.cat((t, feature1, feature2), dim=1)
        else:
            pad_size = self.time_dim - t.size(-1)
            if pad_size > 0:
                t = F.pad(t, (0, pad_size), "constant", 0)
...
class GaussianDiffusion1D(nn.Module):
    ...
    def model_predictions(self, x, t, x_self_cond=None, clip_x_start=False, rederive_pred_noise=False, labels=None):
        model_output = self.model(x, t, x_self_cond, labels=labels)
    ...
    def p_mean_variance(self, x, t, x_self_cond=None, clip_denoised=True, labels=None):
        preds = self.model_predictions(x, t, x_self_cond, labels=labels)
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, x_self_cond=None, clip_denoised=True, labels=None):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((b,), t, device=x.device, dtype=torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(
            x=x, t=batched_times, x_self_cond=x_self_cond, clip_denoised=clip_denoised, labels=labels)
    ...
    def p_sample_loop(self, shape, labels=None):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            ...
            img, x_start = self.p_sample(img, t, self_cond, labels=labels)

        img = self.unnormalize(img)
        return img

    @torch.no_grad()
    def ddim_sample(self, shape, clip_denoised=True, labels=None):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(
                img, time_cond, self_cond, clip_x_start=clip_denoised, labels=labels)
    ...
    @torch.no_grad()
    def sample(self, batch_size=16, labels=None):
        seq_length, channels = self.seq_length, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn((batch_size, channels, seq_length), labels=labels)
...
class Trainer1D(object):
    def __init__(
        ...
        *,
        ...
        self.opt = AdamW(diffusion_model.parameters(), lr=train_lr,
                         betas=adam_betas)  # changed from Adam to AdamW
    ...
    def train(self, sampled_labels=None):
        accelerator = self.accelerator
        device = accelerator.device
        ...
        with tqdm(initial=self.step, total=self.train_num_steps, disable=not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    img_device = img.unsqueeze(1).type(torch.float).to(device)
                    # labels_device = list(map(lambda t: t.to(device), labels))
                    labels_device = [lab.type(torch.float).to(
                        device) for lab in labels]

                    with self.accelerator.autocast():
                        loss = self.model(img_device, labels_device)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        # change torch.save into pickle.save
                        # torch.save(all_samples, str(self.results_folder / f'sample-{milestone}.png'))
                        pickle.dump(all_samples.cpu(), open(
                            str(self.results_folder / f'sample-{milestone}.pkl'), 'wb'))
                        self.save(milestone)

                pbar.update(1)
        # dump all the losses into a pickle file
        pickle.dump(loss_history, open(
            str(self.results_folder / f'loss_history.pkl'), 'wb'))
